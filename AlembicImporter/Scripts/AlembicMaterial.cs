...
public class AlembicMaterial : MonoBehaviour
{
    ...
    public static void Import(string xmlPath, GameObject root, string materialFolder)
    {
        ...
        foreach (XmlNode node in nodes)
        {
            ...
            foreach (XmlNode shader in shaders)
            {
                ...
                Material[] newMaterials = target.GetComponent<Renderer>().sharedMaterials;
                newMaterials[Convert.ToInt32(inst.Value)] = material;

                target.GetComponent<Renderer>().sharedMaterials = newMaterials;
        ...
        /*
        // Force refresh
        // Currently this is breaking the import. Will crash Unity on import on Windows 10 machine(-Bay) 
        AlembicStream abcstream = root.GetComponent<AlembicStream>();
        Debug.Log("ABCSTREAM" +abcstream);
        if (abcstream != null)
        {
    ...
    {
        // Not Yet Implemented
        //if (xmlPath == Application.dataPath+Path.DirectorySeparatorChar)
        {
            //string testPath = Application.dataPath + Path.DirectorySeparatorChar + "test.xml";

            XmlDocument doc = new XmlDocument();

            // valid game object given
            if (root != null)
            {
                var allChildren = root.GetComponentsInChildren(typeof(Renderer));
                XmlElement parentNode = doc.CreateElement("assignments");
                foreach (var child in allChildren)
                {
                    Material[] mats = child.gameObject.GetComponent<Renderer>().sharedMaterials;
                    XmlElement element = doc.CreateElement("node");
                    element.SetAttribute("path", GetGameObjectPath(child.gameObject, root.name));
                    ...
                    if (mats.Length > 0)
                    {
                        ...
                        {
                            if (mat.name != "Material_0 (Instance)")
                            {
                                i++;
                                XmlElement matNode = doc.CreateElement("shader");
                                matNode.SetAttribute("name", mat.name);
                                matNode.SetAttribute("instance", i.ToString());
                                element.AppendChild(matNode);
                                parentNode.AppendChild(element);
                            }
                        }
                    }
                    // Only bother adding the parentNode if there's actually child nodes
                ...
                doc.Save(xmlPath);
                Debug.Log("XML EXPORT TO " + xmlPath);
            }

        }
    #endif
    }
    public static string GetGameObjectPath(GameObject obj, string root)
    {
        string path = "/"+obj.name;
        while (obj.transform.parent != null && obj.transform.parent.name != root)
        {
