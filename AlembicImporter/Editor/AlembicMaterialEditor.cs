...
class AlembicMaterialImporter : EditorWindow
{
    string xmlPath = "";
    ...
    {
        EditorWindow.GetWindow (typeof(AlembicMaterialImporter));
    }
    ...
    string MakeDataRelativePath(string path)
    {
        if (path.Length > 0)
        {
            Uri baseUri = new Uri(Application.dataPath + Path.DirectorySeparatorChar);
    ...
    public void OnGUI()
    {
        ...
        if (GUI.Button(new Rect(r.x+bw/2, r.y, bw, r.height), "Import"))
        {
            AlembicMaterial.Import(Application.dataPath + Path.DirectorySeparatorChar + xmlPath, assetRoot, "Assets" +Path.DirectorySeparatorChar + materialFolder); //relMatFolder);
        }

    }
}

class AlembicMaterialExporter : EditorWindow
{
    string xmlPath = "";
    GameObject assetRoot = null;

    [MenuItem("Assets/Export Alembic Materials")]
    public static void ShowWindow()
    {
        EditorWindow.GetWindow(typeof(AlembicMaterialExporter));
    }
    ...
    public void OnGUI()
    {
        ...
        if (GUI.Button(new Rect(r.x+bw/2, r.y, bw, r.height), "Export"))
        {
            AlembicMaterial.Export(Application.dataPath + Path.DirectorySeparatorChar + xmlPath, assetRoot);
        }
    }
}